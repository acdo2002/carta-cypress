...
const testImageName = 'ibja08020_drc.fits'
...
describe('2D image fitting test:', () => {
    ...
    it(`Open 2D Fitting Dialog and Input Value for fitting`,()=>{
        ...
        cy.get('[placeholder="Center X"]').type('2488')
        cy.get('[placeholder="Center Y"]').type('2106')
        cy.get('[placeholder="Amplitude"]').type('2')
        cy.get('[placeholder="Major Axis"]').type('3')
        cy.get('[placeholder="Minor Axis"]').type('3')
    ...
    it(`Match the 2D fitting result`,()=>{
        cy.get('[class="fitting-result-text"]',{ timeout: 400000 }).should('be.visible')
      })
})